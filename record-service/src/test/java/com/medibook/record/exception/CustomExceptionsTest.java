...
class CustomExceptionsTest {
    ...
    void duplicateResourceExceptionSupportsBothConstructors() {
        DuplicateResourceException formatted = new DuplicateResourceException("MedicalRecord", "appointmentId", 500);
        DuplicateResourceException plain = new DuplicateResourceException("Already exists");

        assertEquals("MedicalRecord already exists with appointmentId: 500", formatted.getMessage());
    ...
    void resourceNotFoundExceptionExposesLookupDetails() {
        ResourceNotFoundException exception = new ResourceNotFoundException("MedicalRecord", "id", 1);

        assertEquals("MedicalRecord not found with id: 1", exception.getMessage());
        assertEquals("MedicalRecord", exception.getResourceName());
