...
public class SlotRequest {
     ...
     */
    @Positive(message = "Provider ID must be greater than 0")
    ...
    @JsonIgnore
    @AssertTrue(message = "Recurrence must be NONE, DAILY, or WEEKLY")
    public boolean isRecurrenceValid() {
        return recurrence == null
                || recurrence.isBlank()
                || "NONE".equalsIgnoreCase(recurrence)
                || "DAILY".equalsIgnoreCase(recurrence)
                || "WEEKLY".equalsIgnoreCase(recurrence);
    }
}
