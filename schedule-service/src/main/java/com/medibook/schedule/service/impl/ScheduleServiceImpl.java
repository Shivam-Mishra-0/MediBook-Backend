...
public class ScheduleServiceImpl implements ScheduleService {
    ...
    private void validateSlotRequest(SlotRequest request) {
        if (request == null) {
            throw new BadRequestException("Slot request cannot be null.");
        }
        ...
        request.setRecurrence(normalizeRecurrence(request.getRecurrence()));
    }
    ...
    private String normalizeRecurrence(String recurrence) {
        ...
        String normalized = recurrence.trim().toUpperCase(Locale.ROOT);
        if (!Set.of("NONE", "DAILY", "WEEKLY").contains(normalized)) {
            throw new BadRequestException(
                "Recurrence pattern must be NONE, DAILY, or WEEKLY."
            );
        }
